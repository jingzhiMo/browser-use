/**
 * TypeScript implementation of judge system
 * Converted from browser_use/agent/judge.py
 */

import * as fs from 'fs';
import * as path from 'path';
import type { BaseMessage, ContentPartImageParam, ContentPartTextParam, ImageURL, SystemMessage, UserMessage } from '../llm/messages';

/**
 * Encode image to base64 string.
 */
function _encode_image(image_path: string): string | null {
	try {
		const pathObj = path.resolve(image_path);
		if (!fs.existsSync(pathObj)) {
			return null;
		}
		const imageData = fs.readFileSync(pathObj);
		return Buffer.from(imageData).toString('base64');
	} catch (error) {
		console.warn(`Failed to encode image ${image_path}: ${error}`);
		return null;
	}
}
 ...
 */
function _truncate_text(text: string, max_length: number, from_beginning: boolean = false): string {
	if (text.length <= max_length) {
		return text;
	}
	if (from_beginning) {
		return '...[text truncated]' + text.slice(-max_length + 23);
	} else {
		return text.slice(0, max_length - 23) + '...[text truncated]...';
	}
}
 ...
 */
export function construct_judge_messages(
	task: string,
	final_result: string,
	agent_steps: string[],
	screenshot_paths: string[],
	max_images: number = 10,
	ground_truth: string | null = null
): BaseMessage[] {
	const task_truncated = _truncate_text(task, 40000);
	const final_result_truncated = _truncate_text(final_result, 40000);
	const steps_text = agent_steps.join('\n');
	const steps_text_truncated = _truncate_text(steps_text, 40000);

	// Select last N screenshots
	const selected_screenshots =
		screenshot_paths.length > max_images ? screenshot_paths.slice(-max_images) : screenshot_paths;

	// Encode screenshots
	const encoded_images: ContentPartImageParam[] = [];
	for (const img_path of selected_screenshots) {
		const encoded = _encode_image(img_path);
		if (encoded) {
			encoded_images.push({
				type: 'image_url',
				image_url: {
...
Evaluate this agent execution given the criteria and respond with the exact JSON structure requested.`;
	...
	const content_parts: (ContentPartTextParam | ContentPartImageParam)[] = [
		{ type: 'text', text: user_prompt } as ContentPartTextParam,
	];
	content_parts.push(...encoded_images);

	return [
		{ role: 'system', content: system_prompt } as SystemMessage,
		{ role: 'user', content: content_parts } as UserMessage,
	];
}
